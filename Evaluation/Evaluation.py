...
class Evaluator:
    ...
    # Visual Evaluation
    #
    def visual_evaluation(self,dataset_evaluation_folder:str = os.environ["EVALUATION_DATASET_FOLDER"],dataset_folder = os.environ["DATASET_FOLDER"]):
        execution_name = "execution_"+datetime.now().strftime("%Y_%m_%d_%H_%M_%S_%p")
        res_path = os.path.join(dataset_evaluation_folder,execution_name)
        pred_vis_path = os.path.join(res_path,'pred_vis')
        safe_make_folder(res_path)
        safe_make_folder(pred_vis_path)
        in_path = dataset_folder
